...
const BENEFITS_VARIANTS = {
    benefits1: {
        ...
        title: 'text-3xl md:text-5xl font-semibold tracking-tight text-gray-900 mb-12 text-center',
        layout: 'accordion'
    },
    benefits2: {
    ...
    benefits4: {
        ...
        title: 'text-3xl md:text-5xl font-semibold tracking-tight text-white mb-12 text-center',
        layout: 'timeline'
    },
    benefits5: {
        section: 'bg-white py-24 md:py-32',
...
export default function Benefits({ settings }: BenefitsProps) {
    ...
    const renderAccordion = () => (
        <div className="space-y-4">
            {benefits.map((benefit: any, index: number) => (
                <div key={index} className="reveal-item border border-gray-200 rounded-lg overflow-hidden">
                    <button
    ...
    const renderList = () => (
        <div className="space-y-6">
            {benefits.map((benefit: any, index: number) => (
                <div key={index} className="reveal-item flex items-start space-x-4 p-6 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    ...
                    <div>
                        <h3 className="text-xl font-semibold tracking-tight text-gray-900 mb-2">{benefit.title}</h3>
                        <p className="text-gray-500">{benefit.description}</p>
                    </div>
                </div>
            ))}
        ...
        <div>
            ...
            <div className="text-center">
                <div className="bg-gray-50 border border-gray-200 p-8 rounded-2xl">
                    <h3 className="text-2xl font-semibold tracking-tight text-gray-900 mb-4">{benefits[activeTab]?.title}</h3>
                    <p className="text-lg text-gray-500">{benefits[activeTab]?.description}</p>
                </div>
            </div>
        </div>
    );
